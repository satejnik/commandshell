...
{
    public class CommandsResolver : ICommandsResolver
    {
        #region Constructor

        private static readonly CommandsResolver Instance = new CommandsResolver();

        private CommandsResolver() { }

        public static CommandsResolver Default
        {
            get
            {
                return Instance;
        ...
        {
            return ResolveFromAssembly(Assembly.GetEntryAssembly() ?? Assembly.GetExecutingAssembly());
        }

        public static IEnumerable<CommandMetadata> ResolveFromAssembly(Assembly assembly)
        {
            return assembly.GetTypes().Where(type => Attribute.IsDefined(type, typeof(ShellCommandAttribute), false) && !type.IsAbstract).Select(GetCommandMetadata);
        }
